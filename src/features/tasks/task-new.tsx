...
export function TaskNewPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      ...
      customerName: selectedPortal?.customerName ?? customer,
  ...
  if (portalsLoading || workflowsLoading) return <MockLoading />;

  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-6">
      <PageHeader title="新建任务" description="创建 Mock 自动化任务" />
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">SRM 与流程模板</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label>SRM 门户 *</Label>
            <Select value={portalId} onValueChange={setPortalId}>
              <SelectTrigger><SelectValue placeholder="选择 SRM 门户" /></SelectTrigger>
              <SelectContent>
                {portals.filter((p) => p.status === "enabled").map((p) => (
          ...
          <div className="space-y-2">
            <Label>流程模板 *</Label>
            <Select value={workflowId} onValueChange={(v) => { setWorkflowId(v); setInputValues({}); }}>
              <SelectTrigger><SelectValue placeholder="选择流程模板" /></SelectTrigger>
              <SelectContent>
                {workflows.filter((w) => w.status === "enabled").map((w) => (
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">执行选项</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <label className="flex items-center gap-2 text-sm">
            <Checkbox checked={confirmBeforeRun} onCheckedChange={(v) => setConfirmBeforeRun(!!v)} />
            执行前确认
          </label>
          <label className="flex items-center gap-2 text-sm">
            <Checkbox checked={saveScreenshots} onCheckedChange={(v) => setSaveScreenshots(!!v)} />
            保存每步截图
          </label>
          <label className="flex items-center gap-2 text-sm">
            <Checkbox checked={enableTrace} onCheckedChange={(v) => setEnableTrace(!!v)} />
            开启 Trace
          </label>
          <label className="flex items-center gap-2 text-sm">
            <Checkbox checked={autoRetry} onCheckedChange={(v) => setAutoRetry(!!v)} />
            失败自动重试
          </label>
        </CardContent>
      </Card>
      ...
      <div className="flex gap-2">
        <Button type="submit" disabled={createMutation.isPending}>保存任务</Button>
        <Button type="button" variant="outline" onClick={() => navigate({ to: "/tasks" })}>取消</Button>
      </div>
    </form>
  );
}
